...
export function addBaseDir(rawPath) {
    const absolute = toAbsolute(rawPath);
    if (!fs.existsSync(absolute) || !fs.statSync(absolute).isDirectory()) {
        printError(`"${absolute}" does not exist or is not a directory.`);
    }
    const config = loadConfig();
    const already = config.baseDirs.some((b) => normalizeForCompare(b) === normalizeForCompare(absolute));
    if (!already) {
...
export function removeBaseDir(rawPath) {
    const config = loadConfig();
...
export function setScanDepth(depth) {
