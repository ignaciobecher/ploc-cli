...
function baseDirRank(entry, baseDirs) {
    const idx = baseDirs.findIndex((b) => b.toLowerCase() === entry.baseDir.toLowerCase());
    return idx === -1 ? baseDirs.length : idx;
}
function findExact(query, entries, baseDirs) {
...
function findPartial(query, entries, baseDirs) {
    const q = query.toLowerCase();
...
function findFuzzy(query, entries, baseDirs) {
    ...
    const withScore = results
        ...
        .map((r) => ({ entry: r.item, tier: 'fuzzy', score: r.score ?? 1 }));
    withScore.sort((a, b) => {
        if (a.score !== b.score)
...
export function findBest(query, entries, config) {
...
export function rankAll(query, entries, config, limit = 15) {
    if (!query) {
        return entries
            .slice()
            .sort((a, b) => a.name.localeCompare(b.name))
            .slice(0, limit)
            .map((entry) => ({ entry, tier: 'partial', score: 0 }));
    }
    const exact = findExact(query, entries, config.baseDirs);
    const partial = findPartial(query, entries, config.baseDirs).filter((p) => !exact.some((e) => e.entry.path === p.entry.path));
    const fuzzy = findFuzzy(query, entries, config.baseDirs).filter((f) => !exact.some((e) => e.entry.path === f.entry.path) && !partial.some((p) => p.entry.path === f.entry.path));
